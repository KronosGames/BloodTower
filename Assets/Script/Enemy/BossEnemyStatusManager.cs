...
public class BossEnemyStatusManager : MonoBehaviour
{
    ...
    /// 状態異常にかかっているかどうか
    /// </summary>
    bool[] isInTheStatus = new bool[(int)BOSS_ENEMY_STATUS.SENTINEL];

    /// <summary>
    /// 状態異常　蓄積値
    /// </summary>
    int[] statusAccumulation = new int[(int)BOSS_ENEMY_STATUS.SENTINEL];

    /// <summary>
    /// 自信のパラメータ
    /// </summary>
    BossEnemyParam thisParam = null;
    ...
    {
        BossEnemyInfo enemyInfo = GetComponent<BossEnemyInfo>();
        if (enemyInfo == null)
        {
            Debug.LogError("BossEnemyInfo is null!");
            return;
        }
    ...
    public void AddStatusAccumulation(BOSS_ENEMY_STATUS enemyStatus, int addValue)
    {
